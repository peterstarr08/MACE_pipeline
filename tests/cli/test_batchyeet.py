...
def test_collect_frames(frames_save_setup_path):
    print(f"Reading from: {frames_save_setup_path}")
    db = collect_frames(frames_save_setup_path)
    assert len(db) == frames_size

def test_export_frames(mocker, frames_save_setup_path, atoms_records, read_io_save_path):
